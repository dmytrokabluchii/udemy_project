...
function calc() {
  ...
  function initLocalSettings(selector, activeClass) {
    ...
    elements.forEach(elem => {
      ...
      elem.classList.remove(activeClass); // Переберем элементы с id и если id будет = значению что наход. в localStorage 'sex'
  ...
  initLocalSettings('.calculating__choose_big div', 'calculating__choose-item_active'); // Фун-я общего вычисления калорий, должна вызыв-я каждый раз когда на стр-це проис-т измен-е

  function calcTotal() {
    // если у нас нет(не введено), т.е. если хот-бы одно их этих условий выпол-ся(будет false)
    if (!sex || !height || !weight || !age || !ratio) {
      // то тогда мы не будем рассчит-ть
      calcResult.textContent = '____'; // далее досрочно прервем фун-ю
  ...
  calcTotal(); // Фун-я получ-я статических данных по калориям и перекл-ю активности, в уроке 98 немного доработали ее

  function getStaticInformation(selector, activeClass) {
    // получаем все div элементы внутри родителя
    const elements = document.querySelectorAll(selector); // Исправ-м баги при клике на поля вне кнопок
    ...
    elements.forEach(elem => {
      ...
      elem.addEventListener('click', e => {
        ...
        if (e.target.getAttribute('data-ratio')) {
          ...
          ratio = +e.target.getAttribute('data-ratio'); // занесем в localStorage записанные полз-м данные
        ...
        e.target.classList.add(activeClass); // В конце каждой из фун-й вызываем calcTotal(), что-бы примен-сь измен-я на стр-це
  ...
  getStaticInformation('#gender div', 'calculating__choose-item_active'); // При 2-м вызове укажем нашего родителя, класс активности при этом не меняется

  getStaticInformation('.calculating__choose_big div', 'calculating__choose-item_active'); // Далее созд-м фун-ю кот-я будет обраб-ть каждый отдел-й input

  function getDinamicInformation(selector) {
    const input = document.querySelector(selector); // Событие input срабатывает каждый раз при изменении значения.
...
function cards() {
  ...
  class MenuCard {
    constructor(src, alt, title, description, price, parentSelector) {
      ...
      this.classes = classes; // в this.parent помещаем DOM элемент(нащего родителя в верстке) кот-й в дальн-м мы сможем испол-ть
      ...
      this.transfer = 27; // вызовем метод ниже changeToUAN() что-бы он уже верн-л нам прав-е значение
    ...
    render() {
      ...
      const element = document.createElement('div'); // если мы ничего не перед-м в ...classes
  ...
  (0,_services_services__WEBPACK_IMPORTED_MODULE_0__.getResource)('http://localhost:3000/menu') // далее обрабат-м наш ответ с сервера, он(data) к нам приходит уже в транфор-м виде объекта(т.е. массива)
  .then(data => {
...
function forms(formSelector, modalTimerId) {
  ...
  const forms = document.querySelectorAll(formSelector); // Создадим объект со списком фраз которые мы будем испол-ть в коде далее. 
  ...
  function bindPostData(form) {
    ...
    form.addEventListener('submit', e => {
      ...
      e.preventDefault(); // Созд-м и показываем блок с соот. сообщение о ...

      let statusMessage = document.createElement('img'); // подгруж-м нашу svg картинку. Атрибут src подгр-м из message.loading

      statusMessage.src = message.loading; // примен-м css стили к картинке
      ...
      form.insertAdjacentElement('afterend', statusMessage); // для отправки данных с формы есть два способа, 1-й: Объект FormData 2-й: JSON формат
      // какой выбрать зависит во многом от сервера(какой формат передачи данных он поддер-т)

      const formData = new FormData(form); // 2-й: JSON формат(в lesson 90 модиц-м код ниже)
      ...
      const json = JSON.stringify(Object.fromEntries(formData.entries())); // Ниже перед .then испол-м нашу фун-ю postDada, во внутрь() кот-й поместим URL-адресс и body который пойдет на сервер
      // где в body мы из обыч. объекта делаем JSON-объект и далее отправ-м его на сервер

      (0,_services_services__WEBPACK_IMPORTED_MODULE_1__.postDada)('http://localhost:3000/requests', json) // теперь нам нужно обработать результат нашего запроса, при помощи промисов
      ...
      .then(data => {
        ...
        showThanksModal(message.success); // чтобы надпись исчезла через время

        statusMessage.remove(); // И что-бы обработ-ть возможные ошибки пропишем метод catch()
      }).catch(() => {
        // если запрос не прошел то
        showThanksModal(message.failure); // И в самом конце пропишем метод finally() для очистки нашей формы после отправки
      }).finally(() => {
  ...
  function showThanksModal(message) {
    ...
    const prevModalDialog = document.querySelector('.modal__dialog'); // скрываем временно блок при помощи стилей

    prevModalDialog.classList.add('hide'); // запустим фун-ю openModal() отвеч-ю за откр-е модал-х окон

    (0,_modal__WEBPACK_IMPORTED_MODULE_0__.openModal)('.modal', modalTimerId); // Созд-м блок-обертку для нашего окна и задаем ему необ. классы

    const thanksModal = document.createElement('div'); // Т.е мы один modal__dialog заменяем другим

    thanksModal.classList.add('modal__dialog'); // Далее просто формир-м верстку кот-я будет наход-ся в данном модал. окне
    ...
    document.querySelector('.modal').append(thanksModal); // и чтобы наше старое модал окно отображалось, удалим thanksModal через 5 сек и соот. покажем prevModalDialog
...
function openModal(modalSelector, modalTimerId) {
  const modalWindow = document.querySelector(modalSelector); // показыв-м и скрываем наше мод. окно, т.е. актив-м соот. css классы + есть вариант с toggle

  modalWindow.classList.add('show', 'fade-modal');
  modalWindow.classList.remove('hide'); // альтерн-й вариант с toggle
...
function closeModal(modalSelector) {
  const modalWindow = document.querySelector(modalSelector); // скрываем и показыв-м наше мод. окно

  modalWindow.classList.add('hide');
  modalWindow.classList.remove('show'); // альтерн-й вариант с toggle
  // modalWindow.classList.toggle('show');
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  ...
  const btnModalOpen = document.querySelectorAll(triggerSelector),
        modalWindow = document.querySelector(modalSelector); //   btnModalClose = document.querySelector('[data-close]');
  ...
  function showModalByScroll() {
    ...
    if (window.pageYOffset + document.documentElement.clientHeight >= document.documentElement.scrollHeight - 1) {
      openModal(modalSelector, modalTimerId); // чтобы при пролистывании до конца окно постоянно не вылазило
...
function slider(_ref) {
  ...
  width = window.getComputedStyle(slidesWrapper).width; // Индекс опред-й полож-е в текущем слайде, где 1 это номер нашего слайда

  let slideIndex = 1; // Для работы с отступами, чтобы мы знали насколько отступили влево-вправо
  ...
  slidesField.style.width = 100 * slides.length + '%'; // Установ-м блоку slidesField css св-во display: flex;
  ...
  slider.style.position = 'relative'; // Создадим блок 'ol' для наших индикаторов и добавим ему класс

  const indicators = document.createElement('ol'),
        dots = [];
  indicators.classList.add('carousel-indicators'); // Вставим Css стили для наших индикаторов испол-я .cssText
  ...
  slider.append(indicators); // Созд-м необ-е нам кол-во буллетов. Цикл закон-ся тогда когда конч-ся слайды(slides.length)

  for (let i = 0; i < slides.length; i++) {
    // Созд-м точки и добавим им атрибут c нумерацией нач-я с 1(i + 1), т.к. 1-й слайд - 1-я точка
    const dot = document.createElement('li');
    dot.setAttribute('data-slide-to', i + 1); // И для красивого отображения добавим им Css стили
    ...
    indicators.append(dot); // Далее мы получим массив с нашими буллетами
  ...
  arrowNext.addEventListener('click', () => {
    ...
    if (offset == doNumberFromString(width) * (slides.length - 1)) {
      offset = 0; // Если это будет не последний слайд, нам нужно будет добавить смещение
    } else {
      // offset +=  +width.slice(0, width.length - 2);
      offset += doNumberFromString(width); // Т.е. когда мы кликаем на стрелку Next к offset добав-я ширина еще одного слайда и слайд смест-ся на опредю велич-у
    }

    transformSlidesLeft(); // При перем-и слайдов мы также должны контр-ть slideIndex в Счетчике. Если мы дошли до конца слайда(slides.length)
  ...
  arrowPrev.addEventListener('click', () => {
    ...
    if (offset == 0) {
      ...
      offset = doNumberFromString(width) * (slides.length - 1); // И если это был не 1-й слайд то знач-е нужно отнимать
    } else {
      offset -= doNumberFromString(width);
    }

    transformSlidesLeft(); // При перем-и слайдов мы также должны контр-ть slideIndex в Счетчике.
    // Если мы на 1-м слайде, то при клике на кнопку пред-го слайда мы будем смещ-ся в самый конец

    if (slideIndex == 1) {
      slideIndex = slides.length; // кол-во слайдов соот. умен-ся при нажатии
    } else {
      slideIndex--;
    }
  ...
  dots.forEach(dot => {
    dot.addEventListener('click', e => {
      const slideTo = e.target.getAttribute('data-slide-to'); // при клике на соот. буллет у нас в slideIndex пойдет соот номер буллета(1 или 5 и тд)

      slideIndex = slideTo; // в пере-ю offset мы запишем наш посл-й слайд вычисл-й по след-й формуле
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  hideTabContent(); // арг-м в() устан-м в фун-и по умолчанию, т.е. 0, в ES6 функ-и мы можем сразу присв-ть 0, (i = 0)
  ...
  tabsParent.addEventListener('click', event => {
    ...
    const target = event.target; // tabsSelector.slice(1) - удалим точку у нашей строки(метод сформ-т новую строку без 1-го символа)
    ...
    if (target && target.classList.contains(tabsSelector.slice(1))) {
      ...
      tabs.forEach((item, i) => {
        ...
        if (target == item) {
          ...
          hideTabContent(); // где в i хранит-ся нужный нам номер по порядку
...
function timer(id, deadline) {
  ...
  function getTimeRemaining(endtime) {
    let days, hours, minutes, seconds; // нам нужно превр-ть строку в нечно осязаемое, испол-м Date.parce
    ...
    const t = Date.parse(endtime) - Date.parse(new Date()); // добавим обработка прошедшей даты(Lesson 70), опред-м будет ли значение у нас отриц-м, 
    ...
    if (t <= 0) {
      ...
      days = Math.floor(t / (1000 * 60 * 60 * 24)); // t / на кол-во мс в 1 часе и так мы получим общее кол-во часов остав-ся до таймера
      ...
      hours = Math.floor(t / (1000 * 60 * 60) % 24); // и по похожему принципу мы будем считать и далее т.к. нам нужны именно "хвостики" а не общее кол-во!

      minutes = Math.floor(t / 1000 / 60 % 60); // тут нам нужно просто разделить кол-во сек внутри мс
    ...
    return {
      ...
      'hours': hours,
      'minutes': minutes,
      'seconds': seconds // в ссылке есть инфа как записать схожие по названию ключи и значения иначе
  ...
  function setClock(selector, endtime) {
    ...
    timeInterval = setInterval(updateClock, 1000); // запус-м updateClock для инициализации(запустит текущ-ю дату) и отмены "моргания" таймера при обновлении страницы

    updateClock(); // 3-я фун-я! обновляющая наш таймер каждую секунду, пишем ее внутри 2-й фун-и setClock
    ...
    function updateClock() {
      ...
      const t = getTimeRemaining(endtime); // 2. теперь нам нужно поместить на стр-цу расчетные величины что мы получили
      ...
      seconds.innerHTML = getZero(t.seconds); // и когда эта фун-я запус-ся, она расч-т нужное нам время и на основ-и этих расчетов запишет на стр-цу 
...
async function getResource(url) {
  ...
  let res = await fetch(url); // Promise возвращаемый вызовом fetch() не перейдёт в состояние "отклонено" из-за ответа HTTP, т.е. не выдаст ошибку!!!
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const modalTimerId = setTimeout(() => (0,_modules_modal__WEBPACK_IMPORTED_MODULE_4__.openModal)('.modal', modalTimerId), 500000); // Lesson 101-102 Модули + Webpuck. Т.к. переменные выше явл-ся функциями - вызовем их

  (0,_modules_tabs__WEBPACK_IMPORTED_MODULE_2__["default"])('.tabheader__item', '.tabcontent', '.tabheader__items', 'tabheader__item_active'); // Установим 2-м аргументом дату дедлайна для таймера

  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_3__["default"])('.timer', '2022-12-08'); // В аргум-ты запишем нужные нам элементы со стр-цы(откр-е мод.окна)

  (0,_modules_modal__WEBPACK_IMPORTED_MODULE_4__["default"])('[data-modal]', '.modal', modalTimerId);
  (0,_modules_cards__WEBPACK_IMPORTED_MODULE_5__["default"])(); // Перед-м в слайдер объект с опред-ми настр-ми(как это делается в плагинах)
