...
class EmployeesAddForm extends Component {
    // Lesson 145. Поля классов и static
    // Испол-я новый систакси полей классов созд-м св-во без испол-я конструктора. Это будет просто св-во класса
    state = {
        name: '',
        salary: ''
    }
    ...
    onSubmit = (e) => {
        e.preventDefault();
        // Можно доработать в будущем блок с новым сотрудником(валидация + оповещение)
        if (this.state.name.length < 2 || !this.state.salary) {
            alert('Введите корректное имя и з/п сотрудника!');
            return;
        }
    ...
    // Lesson 145. static в React. Создадим статичный метод
    static onLog = () => {
        console.log('Hey');
    }
    // Создадим статичное св-во
    static logged = 'on';
...
// Вызовем метод прямо на классе и это сработает т.к. метод onLog() статичен!
...
// Статические методы вызываются через имя класса. Статические методы часто используются для создания 
// вспомогательных функций приложения.